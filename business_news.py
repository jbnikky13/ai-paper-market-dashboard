...
POSITIVE_TERMS = {
    ...
    "acquisition": 4, "merger": 4, "investment": 3, "funding": 3,
...
NOISE_TERMS = {
    "football": -7, "soccer": -7, "celebrity": -7, "movie": -7, "music": -7,
    "actor": -7, "actress": -7, "reality show": -7, "gaming": -6, "videogame": -6,
    "crime drama": -6, "weather": -5, "survival": -6, "sports": -6,
}
...
def _score(title, description="", region="Global"):
    ...
    if any(x in text for x in ("nigeria", "nigerian", "ngx", "lagos", "abuja", "naira")):
        score += 8
    elif region == "Africa" and any(x in text for x in ("africa", "african", "ghana", "kenya", "south africa", "egypt", "morocco")):
        score += 5
    return score
...
def _region(title, description="", forced=None):
    ...
    if any(x in text for x in ("nigeria", "nigerian", "ngx", "lagos", "abuja", "naira")):
        return "Nigeria"
    if any(x in text for x in ("africa", "african", "ghana", "kenya", "south africa", "egypt", "morocco")):
        return "Africa"
    return "Global"
...
def _clean(items, forced_region=None):
    ...
    for a in items:
        ...
        if key in seen:
            continue
        seen.add(key)
        region = _region(title, a.get("description", ""), forced_region)
        score = _score(title, a.get("description", ""), region)
        if score < 3:
            continue
        ...
        try:
            ...
            for item in root.findall("./channel/item"):
                items.append({
                    ...
                    "publishedAt": item.findtext("pubDate") or "",
...
def fetch_market_news(api_key="", limit=8):
    items = _gnews_queries(api_key, GLOBAL_QUERIES)
    provider = "GNews" if items else "Google News RSS"
    if not items:
        items = _rss_queries(GLOBAL_QUERIES)
    return items[:limit], provider


def fetch_africa_business_news(api_key="", limit=8):
    items = _gnews_queries(api_key, AFRICA_QUERIES, "Africa")
    provider = "GNews" if items else "Google News RSS"
    if not items:
        items = _rss_queries(AFRICA_QUERIES, "Africa")
    # Nigeria is intentionally ranked ahead of other African stories when relevance is comparable.
...
def _render_section(title, items, limit):
    ...
    for a in items[:limit]:
        ...
        region = html.escape(a.get("region", "Global"))
    ...
    if len(lines) == 1:
        lines.append("ℹ️ No relevant business stories found.")
    return "\n".join(lines)


def render_telegram(items, limit=8):
    return _render_section("🌍 <b>AFRICA BUSINESS & MARKET NEWS</b>", items, limit)


def render_global_telegram(items, limit=8):
    return _render_section("🌐 <b>GLOBAL MARKET & BUSINESS NEWS</b>", items, limit)
