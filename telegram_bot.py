...
TOKEN=os.getenv("TELEGRAM_BOT_TOKEN","").strip(); CHANNEL=os.getenv("TELEGRAM_CHANNEL_ID","").strip(); NGX_KEY=os.getenv("NGXPULSE_API_KEY","").strip(); CG_KEY=os.getenv("COINGECKO_DEMO_API_KEY","").strip(); GNEWS_KEY=os.getenv("GNEWS_API_KEY","").strip(); VERSION="v5.7"
CG="https://api.coingecko.com/api/v3"; YAHOO="https://query1.finance.yahoo.com/v8/finance/chart"; NGX="https://www.ngxpulse.ng"
CRYPTO={"bitcoin":"BTC","ethereum":"ETH","solana":"SOL","binancecoin":"BNB","ripple":"XRP","dogecoin":"DOGE","chainlink":"LINK","avalanche-2":"AVAX"}; US={"NVDA":"NVIDIA","AMD":"AMD","AVGO":"Broadcom","MSFT":"Microsoft","GOOGL":"Alphabet","AMZN":"Amazon","META":"Meta","TSLA":"Tesla","AAPL":"Apple","QQQ":"Nasdaq-100 ETF","SPY":"S&P 500 ETF"}; NGX_ASSETS={"DANGCEM":"Dangote Cement","GTCO":"GTCO","ZENITHBANK":"Zenith Bank","ACCESSCORP":"Access Holdings","UBA":"UBA","FIRSTHOLDCO":"First HoldCo","MTNN":"MTN Nigeria","AIRTELAFRI":"Airtel Africa","BUAFOODS":"BUA Foods","BUACEMENT":"BUA Cement","SEPLAT":"Seplat Energy","ARADEL":"Aradel Holdings","PRESCO":"Presco","NB":"Nigerian Breweries","FLOURMILL":"Flour Mills"}

def req(url,params=None,headers=None):
    last=None
    for i in range(2):
        try:
            r=requests.get(url,params=params,headers=headers or {"User-Agent":"AI-Market-Intelligence/5.7"},timeout=15)
...
def crypto():
    h={"User-Agent":"AI-Market-Intelligence/5.7"};
    if CG_KEY:h["x-cg-demo-api-key"]=CG_KEY
    r,e=req(f"{CG}/simple/price",{"ids":",".join(CRYPTO),"vs_currencies":"usd","include_24hr_change":"true"},h); return (r.json() if r else {}),e

def yahoo(symbol):
    r,e=req(f"{YAHOO}/{symbol}",{"range":"1mo","interval":"1d","includePrePost":"false"},{"User-Agent":"Mozilla/5.0"})
    if not r:return None,e
    try:
        z=r.json()["chart"]["result"][0]; m=z.get("meta",{}); q=z.get("indicators",{}).get("quote",[{}])[0]; cl=[float(x) for x in q.get("close",[]) if x is not None]; p=m.get("regularMarketPrice") or (cl[-1] if cl else None); prev=m.get("previousClose") or (cl[-2] if len(cl)>1 else None)
...
def ngx():
    primary={}
    if NGX_KEY:
        r,e=req(f"{NGX}/api/ngxdata/stocks",headers={"X-API-Key":NGX_KEY,"Content-Type":"application/json","User-Agent":"AI-Market-Intelligence/5.7"})
        if r:
            try:
                ...
                for x in rows:
                    ...
                    if p is not None:primary[s]={"price":p,"change":ch,"currency":"NGN","name":NGX_ASSETS.get(s,s),"source":"NGX Pulse"}
            except Exception:pass
    if primary:return primary,"LIVE"
    fallback={}
    for s,name in NGX_ASSETS.items():
        x,_=yahoo(f"{s}.LG")
        if x and x.get("price") is not None:x["name"]=name;x["currency"]=x.get("currency") or "NGN";fallback[s]=x
...
def market_lines():
    crypto_rows=[]; stock_rows=[]; c,_=crypto()
    ...
    for s,n in US.items():
        x,_=yahoo(s)
        if x:stock_rows.append({"symbol":s,"name":n,"price":x["price"],"change":x.get("change"),"currency":x.get("currency","USD"),"source":"Yahoo Finance","market":"US / ETFs"})
    n,mode=ngx()
    for s,name in NGX_ASSETS.items():
        x=n.get(s)
    ...
    for x in crypto_rows:
        ch="N/A" if x.get("change") is None else f"{x['change']:+.2f}%"; icon="🟢" if (x.get("change") or 0)>0 else ("🔴" if (x.get("change") or 0)<0 else "🟡"); lines.append(f"{icon} <b>{html.escape(x['name'])}</b> ({x['symbol']}) — {money(x['price'],x['currency'])} ({ch}) • {x['currency']} • {x['source']}")
    lines += ["","<b>📈 STOCK & ETF PRICES</b>","<i>Latest available prices across US/ETFs and NGX.</i>"]
    for x in stock_rows:
        ch="N/A" if x.get("change") is None else f"{x['change']:+.2f}%"; icon="🟢" if (x.get("change") or 0)>0 else ("🔴" if (x.get("change") or 0)<0 else "🟡"); lines.append(f"{icon} <b>{html.escape(x['name'])}</b> ({html.escape(x['symbol'])}) — {money(x['price'],x['currency'])} ({ch}) • {x['market']} • {x['currency']} • {x['source']}")
...
def build_sections():
    global_news,_=fetch_market_news(GNEWS_KEY,limit=4); africa_news,_=fetch_africa_business_news(GNEWS_KEY,limit=4)
...
def split_html(text,limit=3500):
    ...
    for line in text.split("\n"):
        ...
        if len(candidate)>limit and cur:parts.append(cur);cur=line
        else:cur=candidate
...
if __name__=="__main__":
    market,global_html,africa_html=build_sections(); sections=["\n".join(market),global_html,africa_html]; messages=[]
